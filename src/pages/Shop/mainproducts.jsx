...
import { useCart } from '../../ui/CardContext/CartContext'; // Adjust path if needed
import { mockProducts } from '../../data/products'; // Imported Central Array

// Filter only the main shop items or fallback
...
function ProductCard({ product }) {
  ...
  const inCart = cartItems?.has ? cartItems.has(product.id) : false;
...
function PromoBanner() {
  return (
    <div style={{
      ...
      <img src={BANNER_IMAGE} alt="Promo Banner" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0.55 }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(10,6,2,0.85) 0%, rgba(10,6,2,0.5) 55%, rgba(10,6,2,0.1) 100%)' }} />
      <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: 'clamp(28px,5vw,52px)', minHeight: 260, maxWidth: 520 }}>
        <p style={{ fontFamily: 'inherit', fontWeight: 800, fontSize: 11, letterSpacing: 2.5, textTransform: 'uppercase', color: '#f5a058', marginBottom: 14 }}>✦ সেরা মানের পণ্য</p>
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: 'clamp(26px, 4vw, 42px)', lineHeight: 1.25, color: '#fff', marginBottom: 16, textShadow: '0 2px 16px rgba(0,0,0,0.6)' }}>সেরা স্বাদে রান্না<br />হোক স্বস্তিতে</h2>
        <p style={{ fontWeight: 600, fontSize: 14, color: 'rgba(255,255,255,0.78)', lineHeight: 1.65, marginBottom: 24 }}>১০০% প্রাকৃতিক মশলা ও জৈব পণ্য —<br />সরাসরি আপনার দরজায়</p>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {['🌱 Organic', '✓ 100% Pure', '🚚 Fast Delivery'].map(b => (
            <span key={b} style={{ background: 'rgba(255,255,255,0.13)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.22)', color: '#fff', fontWeight: 700, fontSize: 12, padding: '6px 14px', borderRadius: 20 }}>{b}</span>
          ))}
        </div>
      </div>
    </div>
...
function SidebarContent({ activeCategory, setActiveCategory, sortOrder, setSortOrder, maxPrice, setMaxPrice, onReset }) {
  return (
    <div style={{ fontFamily: "'Sora', sans-serif" }}>
      <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1.5, textTransform: 'uppercase', color: '#999', marginBottom: 20 }}>Filter & Sort</p>
      <div style={{ marginBottom: 24 }}>
        <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1.2, textTransform: 'uppercase', color: '#999', marginBottom: 10, display: 'block' }}>Categories</span>
        {CATEGORIES.map(c => (
      ...
      <div style={{ marginBottom: 24 }}>
        <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1.2, textTransform: 'uppercase', color: '#999', marginBottom: 10, display: 'block' }}>Sort by Price</span>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <input type="range" min="50" max="1500" step="10" value={maxPrice} onChange={e => setMaxPrice(Number(e.target.value))} style={{ width: '100%', accentColor: '#00bf63', height: 4, cursor: 'pointer' }} />
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#999', marginTop: 4 }}>
          <span>৳50</span><span>৳1500</span>
        </div>
      </div>
      <button onClick={onReset} style={{ display: 'block', width: '100%', textAlign: 'left', background: 'none', border: '1px solid #ede8e0', fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 600, cursor: 'pointer', color: '#555', padding: '8px 12px', borderRadius: 8, transition: 'all 0.15s' }}>↺ Reset All</button>
    </div>
  );
}
...
export default function ShopSection() {
  ...
  const reset = () => { setActiveCategory('All'); setSortOrder('default'); setMaxPrice(1500); };
  ...
  return (
    <>
      ...
      <div className="shop-sidebar-overlay" style={{ opacity: sidebarOpen ? 1 : 0, pointerEvents: sidebarOpen ? 'all' : 'none', transition: 'opacity 0.25s' }} onClick={() => setSidebarOpen(false)} />
      ...
      <div style={{ background: '#faf6f0', minHeight: '60vh', fontFamily: "'Sora', sans-serif" }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: 'clamp(20px,3vw,28px) clamp(14px,3vw,20px)', display: 'flex', gap: 28, alignItems: 'flex-start' }}>
          <aside className="shop-desktop-sidebar" style={{ width: 230, flexShrink: 0, background: '#fff', borderRadius: 14, border: '1px solid #ede8e0', padding: '24px 18px', position: 'sticky', top: 88 }}>
            <SidebarContent activeCategory={activeCategory} setActiveCategory={setActiveCategory} sortOrder={sortOrder} setSortOrder={setSortOrder} maxPrice={maxPrice} setMaxPrice={setMaxPrice} onReset={reset} />
          </aside>
          <main style={{ flex: 1, minWidth: 0 }}>
            <button className="shop-filter-btn" onClick={() => setSidebarOpen(true)} style={{ display: 'none', alignItems: 'center', gap: 6, background: '#fff3e8', border: '1px solid #e07a2e', color: '#e07a2e', fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: 13, padding: '8px 16px', borderRadius: 9, cursor: 'pointer', marginBottom: 20 }}>☰ Filters</button>
            {filtered.length === 0 ? (
              <div style={{ background: '#fff', border: '1px solid #ede8e0', borderRadius: 14, padding: '60px 20px', textAlign: 'center' }}>
                ...
                <button onClick={reset} style={{ background: 'none', border: 'none', fontFamily: 'inherit', fontWeight: 800, fontSize: 13, color: '#e07a2e', textDecoration: 'underline', cursor: 'pointer' }}>Reset Filters</button>
              </div>
            ) : (
